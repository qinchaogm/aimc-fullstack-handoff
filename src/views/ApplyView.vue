...
import { APPLY_OPTIONS, CONTEST, composeOrganizationAddress, districtsOf } from "@/lib/contest";
...
const existing = computed(() => store.registrations.find((r) => r.userId === store.currentUser?.id) ?? null);
...
function collectErrors(keys: string[]) {
  ...
  if (keys.includes("members")) {
    data.members.forEach((m, i) => {
      const touched = [m.name, m.age, m.gender, m.phone, m.email].some((x) => x.trim());
      if (!touched) return;
      if (!m.name.trim()) next[`members.${i}.name`] = "请填写姓名";
      if (!m.age.trim() || Number.isNaN(Number(m.age))) next[`members.${i}.age`] = "请填写年龄";
      if (!m.gender) next[`members.${i}.gender`] = "请选择性别";
      if (!m.phone.trim() || !PHONE_RE.test(m.phone)) next[`members.${i}.phone`] = "手机号不正确";
      if (!m.email.trim() || !EMAIL_RE.test(m.email)) next[`members.${i}.email`] = "邮箱不正确";
    });
  }
...
function submit() {
  ...
  const members = data.members.filter((m) => [m.name, m.age, m.gender, m.phone, m.email].some((x) => x.trim()));
  ...
  const res = store.submitRegistration({ ...data, members, organizationAddress });
  if (!res.ok) return ElMessage.error(res.error);
  didUpdate.value = updating;
  done.value = res.data;
  ElMessage.success(updating ? "报名信息已更新，已覆盖为最新内容" : "您已经报名成功，可下载报名表");
}
...
function fieldStatus(id: string): GuideStatus {
  const memberError = Object.keys(errors).some((key) => key.startsWith("members.") && errors[key]);
  switch (id) {
    case "direction":
...
async function clearForm() {
  try {
    await ElMessageBox.confirm("将清空本页已填写的全部内容，是否继续？", "清空表单", {
...
<template>
  <div class="flex min-h-screen flex-col">
    ...
    <main class="relative flex-1">
      ...
      >
        <span class="rounded-full border border-amber-300/30 bg-amber-400/10 px-3 py-1 text-xs text-amber-100">报名通道已关闭</span>
        <h1 class="mt-6 text-3xl font-bold text-white">当前为「{{ contestStageLabel(store.contestStage) }}」</h1>
        <p class="mt-3 max-w-md text-sm leading-7 text-slate-300">
          新项目报名已截止。已报名团队请到个人中心下载报名表；提交作品阶段再上传盖章件与压缩包。
        </p>
        <RouterLink to="/dashboard?tab=contest" class="btn-nova mt-8 inline-flex h-11 items-center rounded-xl px-6">
          前往个人中心
        </RouterLink>
      </div>
      ...
      >
        ...
        <div class="mt-8 flex flex-wrap justify-center gap-3">
          <button class="btn-nova inline-flex h-11 items-center gap-2 rounded-xl px-6" @click="downloadRegistrationForm(done!)">
        ...
        <div class="mt-8 flex flex-wrap justify-center gap-3">
          <RouterLink to="/dashboard?tab=contest" class="btn-nova-ghost inline-flex h-10 items-center rounded-xl px-5">查看我的比赛</RouterLink>
      ...
      <div v-else class="mx-auto w-full max-w-[1400px] px-4 py-10 sm:px-6 lg:px-8 sm:py-14">
        <div class="text-center">
          <h1 class="text-2xl font-bold tracking-tight text-white sm:text-3xl">{{ CONTEST.name }} 项目报名</h1>
          <p class="mt-3 text-sm leading-7 text-slate-300">
            标有 <span class="text-rose-300">*</span> 的为必填。全部步骤提交后，再生成并下载官方报名表。
            <template v-if="existing">每位参赛者仅可申报一个项目，再次提交将覆盖为最新报名信息。</template>
          </p>
        </div>
        ...
        <div class="apply-workspace mt-12">
          <div class="apply-workspace-form">
            ...
            <div v-show="step === 0" class="space-y-9">
              <Field id="direction" label="参赛方向" required hint="请选择最匹配的一项。" :error="errors.direction">
                <ChoiceGroup
                  layout="stack"
            ...
            <div v-show="step === 1" class="space-y-9">
              <Field id="leaderName" label="项目负责人" required :error="errors.leaderName">
                <TextField
              ...
              <Field id="leaderTitle" label="负责人职务" required hint="例如：CTO / 项目负责人 / 教授。" :error="errors.leaderTitle">
                <TextField
              ...
              <Field id="contactName" label="单位联系人" required :error="errors.contactName">
                <TextField
              ...
              <Field id="contactPhone" label="联系人电话" required :error="errors.contactPhone">
                <TextField
                  :model-value="data.contactPhone"
                  placeholder="请输入 11 位手机号"
                  @update:model-value="
                    data.contactPhone = $event.replace(/\D/g, '').slice(0, 11);
              ...
              <div id="members" class="scroll-mt-28">
                <div class="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p class="apply-field-hint">选填，0–{{ APPLY_OPTIONS.maxMembers }} 人，可跳过。</p>
                  </div>
                  <button
                    type="button"
                    ...
                    :disabled="data.members.length >= APPLY_OPTIONS.maxMembers"
                    @click="addMember"
                  >
                    <Plus class="size-4" />
                    添加成员
                ...
                <ul v-else class="mt-5 space-y-5">
                  <li v-for="(m, i) in data.members" :key="i" class="rounded-xl border border-blue-400/20 bg-[#071018] p-5">
                    <div class="mb-4 flex items-center justify-between">
                      <p class="text-sm font-medium text-blue-50">成员 {{ i + 1 }}</p>
                      <button
                        type="button"
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                      <Field :id="`members.${i}`" label="姓名" :error="errors[`members.${i}.name`]">
                        <TextField :model-value="m.name" :max-length="20" placeholder="请输入姓名" @update:model-value="m.name = $event" />
                      </Field>
                      <Field :id="`members.${i}.age`" label="年龄" :error="errors[`members.${i}.age`]">
                        <TextField
                          :model-value="m.age"
                          placeholder="请输入年龄"
                          @update:model-value="m.age = $event.replace(/\D/g, '').slice(0, 3)"
                        />
                      </Field>
                      <Field :id="`members.${i}.gender`" label="性别" :error="errors[`members.${i}.gender`]">
                        <ChoiceGroup :model-value="m.gender" :options="APPLY_OPTIONS.genders" :columns="2" @update:model-value="m.gender = $event" />
                      </Field>
                      <Field :id="`members.${i}.phone`" label="手机号码" :error="errors[`members.${i}.phone`]">
                        <TextField
                          :model-value="m.phone"
                          placeholder="请输入 11 位手机号"
                          @update:model-value="m.phone = $event.replace(/\D/g, '').slice(0, 11)"
                        />
                      </Field>
                      <div class="sm:col-span-2">
                        <Field :id="`members.${i}.email`" label="邮箱" :error="errors[`members.${i}.email`]">
                          <TextField :model-value="m.email" placeholder="请输入邮箱" @update:model-value="m.email = $event" />
                        </Field>
                      </div>
                    </div>
                  </li>
            ...
            <div v-show="step === 2" id="review" class="space-y-8 scroll-mt-28">
              <div>
                <p class="apply-field-label">请审核表单内容</p>
                <p class="apply-field-hint">请核对以下信息。确认提交后，系统会根据全部填写内容生成官方报名表，请自行下载。盖章件请在提交作品阶段上传。</p>
              </div>
              <dl class="space-y-4 rounded-xl border border-blue-400/20 bg-[#071018] px-5 py-6 leading-7">
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-400">参赛方向</dt>
                  <dd class="text-right text-blue-50">{{ data.direction }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-400">项目名称</dt>
                  <dd class="text-right text-blue-50">{{ data.projectName }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-400">单位全称</dt>
                  <dd class="text-right text-blue-50">{{ data.organizationName }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-400">单位地址</dt>
                  <dd class="text-right text-blue-50">
                    {{ composeOrganizationAddress(data.organizationCity, data.organizationDistrict, data.organizationDetail) }}
                  </dd>
                </div>
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-400">项目负责人</dt>
                  <dd class="text-right text-blue-50">{{ data.leaderName }} · {{ data.leaderTitle }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-400">单位联系人</dt>
                  <dd class="text-right text-blue-50">{{ data.contactName }} · {{ data.contactPhone }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-400">小组成员</dt>
                  <dd class="text-right text-blue-50">{{ namedMembers().length === 0 ? "未填写（选填）" : `${namedMembers().length} 人` }}</dd>
                </div>
              </dl>
              <div>
                <p class="apply-field-label">项目内容</p>
                <p class="mt-3 whitespace-pre-wrap leading-8 text-slate-200">{{ data.projectContent }}</p>
              </div>
            </div>

            <div id="submit" class="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-blue-400/20 pt-8 scroll-mt-28">
              ...
              <button v-else type="button" class="btn-nova inline-flex h-10 items-center gap-1 rounded-xl px-5" @click="submit">
                <Rocket class="size-4" />
